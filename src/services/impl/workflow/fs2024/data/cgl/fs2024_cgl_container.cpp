...
constexpr std::uint16_t kWideKeyVersion = 21;  ///< vec: u32 key deltas
...
CglContainer ReadCglContainer(const std::string& path) {
    ...
    std::memcpy(&version, head.data() + 0x12, 2);
    ...
    container.sizelessPacked =
        std::filesystem::path(path).filename().string().rfind("dem", 0) == 0;
