...
const char* kUsage =
    ...
    "(--icao ICAO [--bgl FILE.bgl] | "
    "--lat N --lon N [--osm-json FILE.json] [--heading N]) "
    "[--tile-size M] [--extent M] [--spacing M] [--texture-per-tile PX] "
    "[--cgl-root DIR] [--landmark-catalog FILE.json] "
...
PrepareArgs ParsePrepareArgs(int argc, char** argv) {
    ...
    for (int i = 1; i < argc; ++i) {
        ...
        if (flag == "--icao") args.icao = Next(argc, argv, i);
            ...
            args.heading = std::stof(Next(argc, argv, i));
        ...
        else if (flag == "--cgl-root")
            args.cglRoot = Next(argc, argv, i);
        else if (flag == "--landmark-catalog")
            args.landmarkCatalog = Next(argc, argv, i);
        else if (flag == "--pgg-textures")
            args.pggTextures = Next(argc, argv, i);
