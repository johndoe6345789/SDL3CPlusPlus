...
WorkflowAudioPlayStep::WorkflowAudioPlayStep(
...
void WorkflowAudioPlayStep::Execute(const WorkflowStepDefinition& step,
    ...
    const std::string mode = NormalizeAudioPlayMode(
        ResolveAudioPlayMode(step, context, parameterResolver));
    const bool loop =
        ResolveAudioPlayLoop(step, context, parameterResolver, false);
    ...
    if (mode == "background" || mode == "music") {
        ...
        throw std::runtime_error(
            "audio.play mode must be 'background' or 'effect'");
    }

    if (logger_) {
        logger_->Trace("WorkflowAudioPlayStep", "Execute",
                       "path=" + pathValue.string() + ", mode=" + mode +
