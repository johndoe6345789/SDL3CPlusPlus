...
WorkflowCompareLteStep::WorkflowCompareLteStep(std::shared_ptr<ILogger> logger)
    : logger_(std::move(logger)) {}

std::string WorkflowCompareLteStep::GetPluginId() const {
    return "compare.lte";
}

void WorkflowCompareLteStep::Execute(const WorkflowStepDefinition& step, WorkflowContext& context) {
    ...
    if (!left || !right) {
        throw std::runtime_error("compare.lte requires number inputs");
    }
    ...
    if (logger_) {
        logger_->Trace("WorkflowCompareLteStep", "Execute",
                       "left=" + leftKey +
                           ", right=" + rightKey +
                           ", output=" + outputKey,
