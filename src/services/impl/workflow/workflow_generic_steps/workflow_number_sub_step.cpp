...
WorkflowNumberSubStep::WorkflowNumberSubStep(std::shared_ptr<ILogger> logger)
    : logger_(std::move(logger)) {}

std::string WorkflowNumberSubStep::GetPluginId() const {
    return "number.sub";
}

void WorkflowNumberSubStep::Execute(const WorkflowStepDefinition& step, WorkflowContext& context) {
    ...
    if (!left || !right) {
        throw std::runtime_error("number.sub missing inputs '" + leftKey + "' or '" + rightKey + "'");
    }
    ...
    if (logger_) {
        logger_->Trace("WorkflowNumberSubStep", "Execute",
                       "left=" + std::to_string(*left) +
                           ", right=" + std::to_string(*right) +
                           ", output=" + outputKey,
                       "Subtracted workflow numbers");
    }
}
