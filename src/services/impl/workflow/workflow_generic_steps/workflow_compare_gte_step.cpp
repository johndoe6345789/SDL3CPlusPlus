...
WorkflowCompareGteStep::WorkflowCompareGteStep(std::shared_ptr<ILogger> logger)
    : logger_(std::move(logger)) {}

std::string WorkflowCompareGteStep::GetPluginId() const {
    return "compare.gte";
}

void WorkflowCompareGteStep::Execute(const WorkflowStepDefinition& step, WorkflowContext& context) {
    ...
    if (!left || !right) {
        throw std::runtime_error("compare.gte requires number inputs");
    }
    ...
    if (logger_) {
        logger_->Trace("WorkflowCompareGteStep", "Execute",
                       "left=" + leftKey +
                           ", right=" + rightKey +
                           ", output=" + outputKey,
