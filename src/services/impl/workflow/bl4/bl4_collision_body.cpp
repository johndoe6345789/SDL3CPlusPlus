...
bool IsUnitScale(const glm::vec3& scale) {
    ...
    return std::fabs(scale.x - 1.f) < kEpsilon && std::fabs(scale.y - 1.f) < kEpsilon &&
...
bool AddBl4InstanceBody(btDiscreteDynamicsWorld* world, const Bl4Placement& placement,
    ...
    if (!IsUnitScale(placement.scale)) {
        instance.scaledShape = new btScaledBvhTriangleMeshShape(
            ...
            btVector3(placement.scale.x, placement.scale.y, placement.scale.z));
    ...
    transform.setOrigin(
        btVector3(placement.position.x, placement.position.y, placement.position.z));
    transform.setRotation(btQuaternion(placement.rotation.x, placement.rotation.y,
