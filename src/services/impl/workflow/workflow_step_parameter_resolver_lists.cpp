...
std::vector<std::string> WorkflowStepParameterResolver::GetRequiredStringList(
    ...
    if (logger_) {
        logger_->Trace("WorkflowStepParameterResolver", "GetRequiredStringList",
                       "Entry");
    }
...
std::vector<double> WorkflowStepParameterResolver::GetRequiredNumberList(
    const WorkflowStepDefinition& step, const std::string& name) const {
    if (logger_) {
        logger_->Trace("WorkflowStepParameterResolver", "GetRequiredNumberList",
                       "Entry");
    }
    const auto& param = GetRequiredParameter(step, name);
    if (param.type != WorkflowParameterValue::Type::NumberList) {
        throw std::runtime_error("Workflow step '" + step.id + "' parameter '" +
                                 name + "' must be number list");
    }
