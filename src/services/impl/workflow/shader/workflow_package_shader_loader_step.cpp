...
#include "services/interfaces/workflow/bootstrap/package_shader_load.hpp"
...
WorkflowPackageShaderLoaderStep::WorkflowPackageShaderLoaderStep(
    std::shared_ptr<ILogger> logger, const std::string& gamePackage,
...
void WorkflowPackageShaderLoaderStep::Execute(
    ...
    if (result.status == "success") {
        ...
        context.Set<std::string>("shader.package_json_path",
                                 result.packageJsonPath);
    } else if (result.status == "error") {
        context.Set<std::string>("shader.error_message", result.errorMessage);
    }
