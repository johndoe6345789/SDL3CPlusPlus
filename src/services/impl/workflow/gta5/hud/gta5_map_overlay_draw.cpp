...
void DrawGta5MapOverlay(Gta5MapOverlay& map, SDL_GPUDevice* device,
    ...
    for (const Gta5MapRange& range : frame.ranges) {
        // Unclipped is a rectangle bigger than any screen.
        const bool clip = range.clip.z > 0.f;
        const SDL_Rect scissor =
            clip ? SDL_Rect{int(range.clip.x), int(range.clip.y),
                            int(range.clip.z), int(range.clip.w)}
                 : SDL_Rect{0, 0, 16384, 16384};
