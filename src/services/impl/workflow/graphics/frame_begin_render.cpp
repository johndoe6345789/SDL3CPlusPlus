...
FrameClearColor ParseFrameClearColorOrThrow(
    ...
    if (!clearColorJson || !clearColorJson->is_array() ||
        ...
        throw std::runtime_error("graphics.frame.begin requires "
...
SDL_GPUCommandBuffer* AcquireFrameCommandBufferOrThrow(
...
SwapchainAcquireResult AcquireSwapchainTextureOrThrow(
    ...
    if (!SDL_WaitAndAcquireGPUSwapchainTexture(
            cmd, window, &result.texture, &result.width, &result.height)) {
...
SDL_GPUTexture* GetOrCreateFrameDepthTexture(
    ...
    SDL_GPUTexture* depth_texture =
        SDL_CreateGPUTexture(device, &depth_info);
    if (!depth_texture) {
        throw std::runtime_error(
