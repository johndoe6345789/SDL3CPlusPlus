...
/// Appends one building's walls and its roof (BspRenderVertex, the
...
///
/// A wall's u tiles every `kWallTextureMetres` (one window bay) and
/// its v every `kStoreyTextureMetres` (one storey), which is how the
/// facade texture baked from FS2024's own generator assets is laid
/// out -- one bay wide, one storey tall, window included.
///
...
void AppendBuildingMesh(
    const std::vector<Point2>& footprint, float height, RoofShape roof,
    float roofRise,
